...
def price(request):
  ...
  return HttpResponse(Response(m=[oriPrice / 10.0, newOrder.price / 10.0]).toJson(), content_type="application/json")
...
def successOrder(order, status, time_end):
  ...
  # 2.1 更新该用户的优惠券数量
  priceBK = order.price
  getOrderPrice(order, (order.end_time - order.start_time).seconds / 60)
  order.price = priceBK
